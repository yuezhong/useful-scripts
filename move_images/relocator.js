...
fs.readdir(srcDir, function(err, files) {
    ...
    files.forEach((item) => {
        ...
        if(regex.test(item) ) {
            let pos = item.search("_20");
            let year = item.slice(pos+1, pos+5);
            let month = item.slice(pos+5,pos+7);

            // Get rid of leading zero for months prior to october
            if(month.at(0) == 0) {
                month = month.slice(1);
            }
            ...
            // Create month folder
            if(!fs.existsSync(`${destDir}/${year}/${months[month]}`)) {
                fs.mkdirSync(`${destDir}/${year}/${months[month]}`);
            }

            // Copy files from original folder to the destination folder
            fs.copyFileSync(`${srcDir}/${item}`, `${destDir}/${year}/${months[month]}/${item}`);
            console.log("year: " + year + " month: " +  month + " " + months[month]);
        }
